...
class Bixel(DriverBase):
    ...
    def __init__(self, type=LEDTYPE.APA102, num=256, dev="",
                 c_order=ChannelOrder.RGB, SPISpeed=2, gamma=None,
    ...
    def _sync(self):
        packet = Bixel._generateHeader(CMDTYPE.SYNC, 0)
        self._com.write(packet)
        resp = self._com.read(1)
        if len(resp) == 0:
            Bixel._comError()
...
class DriverTeensySmartMatrix(Bixel):
